/*-------------------------------------------------------------------------
 *
 * Java Foreign Data Wrapper for PostgreSQL
 *
 ...
 * SQL functions
 */
extern Datum java_fdw_handler(PG_FUNCTION_ARGS);
extern Datum java_fdw_validator(PG_FUNCTION_ARGS);
...
#else /* 9.1 only */
static FdwPlan *javaPlanForeignScan(Oid foreigntableid, PlannerInfo *root, RelOptInfo *baserel);
#endif
...
static void javaReScanForeignScan(ForeignScanState *node);

static void javaEndForeignScan(ForeignScanState *node);
 ...
 *
 * These next structures are not actually used by java,but something like
...
/*
 * The plan state is set up in javaGetForeignRelSize and stashed away in
 * baserel->fdw_private and fetched in javaGetForeignPaths.
 */
typedef struct
{
	char	   *foo;
 ...
 *
 * It is set up in javaBeginForeignScan and stashed in node->fdw_state
 * and subsequently used in javaIterateForeignScan,
 * javaEndForeignScan and javaReScanForeignScan.
 */
typedef struct
{
	char	   *baz;
 ...
 *
 * It is set up in javaBeginForeignModify and stashed in
 * rinfo->ri_FdwState and subsequently used in javaExecForeignInsert,
 * javaExecForeignUpdate, javaExecForeignDelete and
 * javaEndForeignModify.
 */
typedef struct
{
	char	   *chimp;
...
java_fdw_handler(PG_FUNCTION_ARGS)
{
	...
	fdwroutine->ImportForeignSchema = javaImportForeignSchema;

	/* Support for scanning foreign joins */
	fdwroutine->GetForeignJoinPaths = javaGetForeignJoinPaths;

	/* Support for locking foreign rows */
	fdwroutine->GetForeignRowMarkType = javaGetForeignRowMarkType;
	fdwroutine->RefetchForeignRow = javaRefetchForeignRow;
	...
	if (list_length(options_list) > 0)
		ereport(ERROR,
				(errcode(ERRCODE_FDW_INVALID_OPTION_NAME),
				 ...
				 errhint("Java FDW does not support any options")));
							...
							int eflags)
{
	...
	JavaFdwModifyState *modify_state =
		palloc0(sizeof(JavaFdwModifyState));
						   ...
						   TupleTableSlot *planSlot)
{
	...
	/* ----
	 * JavaFdwModifyState *modify_state =
	 *	 (JavaFdwModifyState *) rinfo->ri_FdwState;
	 * ----
	 */
...
static int
javaIsForeignRelUpdatable(Relation rel)
{
	/*
